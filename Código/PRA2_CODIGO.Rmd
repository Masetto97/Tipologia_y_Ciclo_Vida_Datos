...
write.csv(datos, "./heart_dissease_data_clean.csv")
```
...
for (i in colnames(datos)) {
  ...
  {
    p_val = ad.test(unlist(datos[i]))$p.value
    if (p_val < alpha) {
      cat(i)
      # Format output
...
```{r}
# Edad
variables <- c("Age", "RestingBP", "Cholesterol",
               "FastingBS", "MaxHR", "Oldpeak", 
               "HeartDisease")
for(i in(variables))
{
  qqnorm(unlist(datos[i]),  main = paste0("Q-Q para la variable: ", i));qqline(unlist(datos[i]), col = 2)
}
