...
```{r}
datos_bp.colesterol <- boxplot(datos_final$COLESTEROL);dev.off()
...
<<<<<<< HEAD
datos_frec.cardiaca.max <- boxplot(datos_final$`FREC CARDÍACA MÁX`);dev.off()
...
dev.off()
>>>>>>> 531df414240c8a3f4eb9ef494fd7b5284e226d0c

datos_oldpeak <- boxplot(datos_final$OLDPEAK);dev.off()
...
<<<<<<< HEAD
datos_bp.presion_arterial <- boxplot(datos_final$`PRESIÓN ARTERIAL`);dev.off()
...
datos_bp.tipo_dolor_torax <- boxplot(datos_final$`TIPO DOLOR TORAX`);dev.off()
