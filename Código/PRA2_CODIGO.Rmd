...
for (i in colnames(datos_final))
{
  ...
  if(i != "E. CARDIACA")
  {
    tabla_cruzada <- table(as.numeric(unlist(datos_final[i])), datos_final$`E. CARDIACA`)
...
set.seed(666)
y <- datos_final$`E. CARDIACA` # Variable objetivo
...
Regla: 1 -> PENDIENTE ST = 0 -> Tendrán menos probabilidades de sufrir E. CARDIACA
Regla: 2 -> PENDIENTE ST in {1, 2} -> Tendrán más probabilidades de sufrir E. CARDIACA
...
```{r message= FALSE, warning=FALSE}
# RENOMBRAMOS VARIABLES PARA PODER DIBUJAR:
train_X2 <- train_X
colnames(train_X2)[1] <- "TIPO_DOLOR_TORAX"
colnames(train_X2)[3] <- "FREC_CARD_MAX"
colnames(train_X2)[5] <- "PENDIENTE_ST"
model <- C50::C5.0(train_X2, train_y)
...
Verdaderos Negativos (E. CARDIACA): 110
Verdaderos Positivos (E. CARDIACA): 123
Falsos Negativos (E. CARDIACA): 28
Falsos Positivos (E. CARDIACA): 25
...
```{r}
# Antes de aplicar el modelo random forest con el conjunto de variables obtenidos para el arbol de decision es necesario discretizar las variables COLESTEROL y FREQ CARDICA MAX
# Establcemos los siguientes niveles:
# COLESTEROL [ COL: MENOR DE 200 -> 0, COL: ENTRE 200 - 400 -> 1, COL: ENTRE 400 - 600 -> 2, COL: MAYOR QUE 600 -> 3]
...
```{r}
colnames(train_X2)
train.data <- as.data.frame(cbind(train_X2[c(1,2,5,4)] ,train_y))
colnames(train.data)[5] <- "ENF_CARDIACA"
train.data
rf <-  randomForest(ENF_CARDIACA ~ ., data = train.data, ntree = 50)

X <- train.data[which(names(train.data) != "ENF_CARDIACA")]
predictor <- Predictor$new(rf, data = X, y = train.data$ENF_CARDIACA) 
